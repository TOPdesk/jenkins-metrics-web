...
function getTestResultMetrics(conn, jobName) {
  return getTestResultData(jobName)
    .without('timestamp')
    .reduce(toTestResultTypes)
    .default(getEmptyTestResultDistribution())
    .merge(withTestResultRatio)
    .run(conn);
}

function getEarliestTestResultMetrics(conn, jobName) {
  const percent = config.has(TEST_RESULTS_EARLIEST_LIMIT)
    ? config.get(TEST_RESULTS_EARLIEST_LIMIT)
    : DEFAULT_LIMIT_PERCENT;
  return getLimitedTestResultMetrics(jobName, percent, rethink.asc('timestamp'))
    .run(conn);
}

function getLatestTestResultMetrics(conn, jobName) {
  const percent = config.has(TEST_RESULTS_EARLIEST_LIMIT)
    ? config.get(TEST_RESULTS_EARLIEST_LIMIT)
    : DEFAULT_LIMIT_PERCENT;
  return getLimitedTestResultMetrics(jobName, percent, rethink.desc('timestamp'))
    .run(conn);
}

function getLimitedTestResultMetrics(jobName, percent, order) {
...
function toTestResultTypes(left, right) {
  return {
    ...
    total: left.getField('total').add(right.getField('total'))
  };
}
...
function withTestResultRatio(val) {
  return rethink.branch(val('total').ne(0),
    {
      failureRatio: val('failed').div(val('total')),
      successRatio: val('successful').div(val('total')),
      skippedRatio: val('skipped').div(val('total'))
    },
    {
      failureRatio: 0,
      successRatio: 0,
...
module.exports = {
  ...
  getEarliestTestResultMetrics,
  getLatestTestResultMetrics
};
