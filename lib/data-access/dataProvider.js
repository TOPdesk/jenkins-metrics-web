...
function toMergedTestResults(val) {
  return val
    .merge({
      failed: val.getField('actions')
        .reduce(sumField('failCount'))
        .default(0)
    })
    .merge({
      skipped: val.getField('actions')
        .reduce(sumField('skipCount'))
        .default(0)
    })
    .merge({
      total: val.getField('actions')
        .reduce(sumField('totalCount'))
        .default(0)
    });
}
...
function sumField(fieldName) {
  return (left, right) =>
    left.getField(fieldName)
      .add(right.getField(fieldName));
}
